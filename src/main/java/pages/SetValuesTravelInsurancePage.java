...
public class SetValuesTravelInsurancePage {
    ...
    public SetValuesTravelInsurancePage(){
        PageFactory.initElements(BaseSteps.getDriver(), this);
    }

    public void setRegion (String itemName){
        BaseSteps.getDriver().findElement(By.xpath("//div[contains(@class,'form-control__select')]")).click();
        BaseSteps.getDriver().findElement(By.xpath("//a[contains(text(),'"+itemName+"')]")).click();
    }

    public void setSum (String itemName){
        BaseSteps.getDriver().findElement(By.xpath("//h3[contains(text(),'"+itemName+"')]")).click();
    }
    ...
    public void fillFieeld(String fieldName, String value) throws InterruptedException {
        switch (fieldName) {
            case "ФамилияTo":
    ...
    public void checkErrorMessage(String waitMessage){
        assertEquals(waitMessage, BaseSteps.getDriver().findElement(By.xpath("//div[contains(@class,'alert-form alert-form-error')]")).getAttribute("innerText"));
    }

    public void checkCellErrorMessage(String waitMessage, String cellName){
        assertEquals(waitMessage, BaseSteps.getDriver().findElement(By.xpath("//form-control-label[contains(@title,'"+cellName+"')]//span[contains(@class,'message')]")).getAttribute("innerText"));
    }
    public void checkFemale (String feMale){
        switch (feMale) {
            case "Мужской":
                if (!BaseSteps.getDriver().findElements(By.xpath("//label[contains(@class,'btn ng-untouched ng-pristine ng-valid active')][contains(text(),'Женский')]")).isEmpty()) {
                    BaseSteps.getDriver().findElement(By.xpath("//label[contains(@class,'btn ng-untouched ng-pristine ng-valid')][contains(text(),'Мужской')]")).click();
                }
                break;
            case "Женский":
                if (!BaseSteps.getDriver().findElements(By.xpath("//label[contains(@class,'btn ng-untouched ng-pristine ng-valid active')][contains(text(),'Мужской')]")).isEmpty()) {
                    BaseSteps.getDriver().findElement(By.xpath("//label[contains(@class,'btn ng-untouched ng-pristine ng-valid')][contains(text(),'Женский')]")).click();
                }
                break;
        }
    }
